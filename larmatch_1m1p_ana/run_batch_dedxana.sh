...
LARMATCH_DEDXANA_DIR=/cluster/tufts/wongjiradlab/twongj01/ubdl-ana/larmatch_1m1p_ana/
...
for ((i=0;i<${STRIDE};i++)); do
    ...
    larmatch_basename=$(echo $baseinput | sed 's|dlfilter_|larmatch_kps_|g' | sed 's|.root||g' | xargs -I{} echo {}"-${jobname}")
    ...
    # KPSRECO FILENAME
    kpsreco_basename=$(echo $baseinput | sed 's|dlfilter_|kpsreco_|g' | sed 's|.root||g' | xargs -I{} echo {}"-${jobname}")
    kpsreco_rootname=${kpsreco_basename}_kpsrecomanagerana.root
    kpsreco_path=${KPSRECO_OUTDIR}/${subdir}/${kpsreco_rootname}

    # DEDX-ANA FILENAME
    dedxana_basename=$(echo $baseinput | sed 's|dlfilter_|dedxana_|g' | sed 's|.root||g' | xargs -I{} echo {}"-${jobname}")
