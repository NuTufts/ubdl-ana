...
LARMATCH_DEDXANA_DIR=${UBDL_DIR}/larflow/larflow/Ana/
...
for ((i=0;i<${STRIDE};i++)); do
    ...
    # DEDX-ANA FILENAME
    dedxana_basename=$(echo $baseinput | sed 's|merged_dlreco|dedxana|g' | sed 's|.root||g' | xargs -I{} echo {}"-${jobname}")
    ...
    CMD="${LARMATCH_DEDXANA_DIR}/./dedx_larmatch_from_true_tracks ${inputfile} ${larmatch_larlite_path} ${dedxana_rootname}"
    echo $CMD
