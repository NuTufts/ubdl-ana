...
for ((i=0;i<${STRIDE};i++)); do
    ...
    # copy input
    cp $inputfile $baseinput   # larcvtruth
    cp $mcinfofile $mcinfoname # mcinfo file
    ...
    CMD2="python $SPEMBED_DIR/prep_spatialembed_from_larflowtruth.py --input-larcv $baseinput --input-larlitetruth $mcinfoname -adc wiremc -o $spembedout --nu-only"
    echo $CMD2
