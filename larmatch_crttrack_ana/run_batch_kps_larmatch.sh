...
for ((i=0;i<${STRIDE};i++)); do
    ...
    local_outfile=$(echo $baseinput | sed 's|merged_dlana|larmatch_kps|g' | sed 's|.root||g' | xargs -I{} echo {}"-${jobname}.root")
    local_basename=$(echo $baseinput | sed 's|merged_dlana|larmatch_kps|g' | sed 's|.root||g' | xargs -I{} echo {}"-${jobname}")
