...
WORKDIR=/cluster/tufts/wongjiradlab/twongj01/ubdl-ana/va_study/
UBDL_DIR=/cluster/tufts/wongjiradlab/twongj01/ubdl_py3
RECO_TEST_DIR=${UBDL_DIR}/larflow/larflow/Reco/test/
OUTPUT_DIR=/cluster/tufts/wongjiradlab/twongj01/ubdl-ana/va_study/output/${SAMPLE_NAME}/vareco/larlite
OUTPUT_ANA_DIR=/cluster/tufts/wongjiradlab/twongj01/ubdl-ana/va_study/output/${SAMPLE_NAME}/vareco/ana
...
# LOCAL JOBDIR
local_jobdir=`printf /tmp/vareco_jobid%d_%04d_${SAMPLE_NAME} ${SLURM_JOB_ID} ${SLURM_ARRAY_TASK_ID}`
#echo "local jobdir: $local_jobdir"
rm -rf $local_jobdir
mkdir -p $local_jobdir

# local log file
local_logfile=`printf vareco_${SAMPLE_NAME}_jobid%d_%04d.log ${SLURM_JOB_ID} ${SLURM_ARRAY_TASK_ID}`
...
for ((i=0;i<${STRIDE};i++)); do
    ...
    local_outfile=$(echo $baseinput  | sed 's|'"${INPUTSTEM}"'|vareco_'"${fileidname}"'|g')
    local_basename=$(echo $baseinput | sed 's|'"${INPUTSTEM}"'|vareco_'"${fileidname}"'|g' | sed 's|.root||g')
    echo "outfile : "$local_outfile >> ${local_logfile}
    scp $inputfile $baseinput
    scp $lminput $baselm
    
    CMD="python $RECO_TEST_DIR/dev_nuvertexactivity.py --input-dlmerged  $baseinput --input-larflow $baselm --input-mcinfo=$baseinput --output $local_outfile"
