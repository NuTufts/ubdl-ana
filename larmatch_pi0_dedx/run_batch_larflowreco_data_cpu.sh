...
OUTPUT_DIR=${WORKDIR}/output_reco/data/larlite/
OUTPUT_ANA_DIR=${WORKDIR}/output_reco/data/ana/
OUTPUT_LOGDIR=${WORKDIR}/logdir/reco/data/
...
for ((i=0;i<${STRIDE};i++)); do
	...
	CMD="python $RECO_TEST_DIR/run_kpsrecoman.py --input-dlmerged ${baseinput}  --input-larflow ${baselm} --output ${local_outfile} -tb --products rerun "
